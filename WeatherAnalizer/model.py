...
from layers import CustomEmbeddingLayer, CustomWeatherProcessingLayer
...
class OutfitWeatherModel(keras.Model):
    def __init__(self, num_types, num_tags):
        ...
        # Tags processing
        self.tags_embedding = CustomEmbeddingLayer(
            input_dim=num_tags, 
            output_dim=64, 
            aggregation="sum"
        )
    ...
    def call(self, inputs):
        ...
        # Combine features
        combined = tf.concat([weather_embed, tags_embed, types_embed], axis=-1)
...
model.fit([weather_data, tags_data, types_data], labels, epochs=10, batch_size=32, validation_split=0.2, callbacks=[early_stopping])
print(model.summary())